...
void Iori_2P::Render(HDC hdc)
{
	...
	if (iori_SidePosition == 2)
	{
		...
		{
			Rectangle(hdc, rcHit.left, rcHit.top, rcHit.right, rcHit.bottom);
			img[0].RenderFlip(hdc, pos.x-img[0].GetImageInfo()->frameWidth, pos.y, frameCount);
		}

		if (ioriState == Chractor_STATE::BACK)
		{
			Rectangle(hdc, rcHit.left, rcHit.top, rcHit.right, rcHit.bottom);
			img[1].RenderFlip(hdc, pos.x-img[1].GetImageInfo()->frameWidth, pos.y, frameCount);
		}

		if (ioriState == Chractor_STATE::STAND)
		{
			Rectangle(hdc, rcHit.left, rcHit.top, rcHit.right, rcHit.bottom);
			img[2].RenderFlip(hdc, pos.x- img[2].GetImageInfo()->frameWidth, pos.y, frameCount);
		}
