...
HRESULT KimKaphwan_1P::Init()
{
	...
	if (FAILED(kim_bar_1P->Init("Image/kim_1P_selected.bmp", WINSIZE_X, WINSIZE_Y, 1, 1, true, RGB(0, 0, 0))))
	{
...
void KimKaphwan_1P::Update()
{
	...
	if (!canInput && (kimState == State::SHAND || kimState == State::SFOOT || kimState == State::COMBO
		|| kimState == State::WFOOT || kimState == State::WHAND /*|| kimState == State::HIT*/ || kimState == State::INIT))
	{
		FrameCheck();
...
void KimKaphwan_1P::Render(HDC hdc)
{
	...
	if (kim_SidePosition == 1)
	{
		...
		case State::DOWN:
			...
			break;
		case State::INIT:
			init->Render(hdc, pos.x - 20, pos.y, frameCount);
			break;
		case State::WIN:
