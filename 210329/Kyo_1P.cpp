...
HRESULT Kyo_1P::Init()
{
	kyo_SidePosition = 1;	//1p의 초기값은 오른쪽을 바라본다 
	...
	{
		MessageBox(g_hWnd, "타격 로드 실패", "경고", MB_OK);
		return E_FAIL;
	}
	...
	{
		MessageBox(g_hWnd, "등장 로드 실패", "경고", MB_OK);
		return E_FAIL;
	}
	...
	if (FAILED(kyo_bar_1P->Init("Image/kyo_1P_selected.bmp", WINSIZE_X, WINSIZE_Y, 1, 1, true, RGB(0, 0, 255))))
	{
...
void Kyo_1P::Motion1P()
{
	...
	if (KeyManager::GetSingleton()->IsStayKeyDown('N') && canInput)
	{
		...
		if (kyo_SidePosition == 2)	//1p가 왼쪽을 바라보는 경우
		{
			kyoState = State::BACK;
			characterFrame = 6;
		}
...
void Kyo_1P::Render(HDC hdc)
{
	...
	if (kyo_SidePosition == 1)	//1p가 오른쪽을 바라보는 경우
	{
		...
		case State::DOWN:
			...
			break;
		case State::INIT:
			kyo_init->Render(hdc, pos.x - 70, pos.y - 45, frameCount);
			break;
		case State::WIN:
